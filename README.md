...
- A separate SQS worker for packet generation, notifications, and readiness calculation, with three-attempt retry, DLQ redrive, and operator recovery
- Real PDFKit event packets stored as private S3 objects and downloaded through five-minute signed URLs
...
The API runs checked-in migrations automatically and never uses TypeORM schema synchronization. LocalStack creates the private packet bucket, work queue, and dead-letter queue. The seeded database contains synthetic demo data only.
...
3. Publish revision 4; watch the separate worker generate and store the event packet under Operations.
4. Switch to the member view and acknowledge the changed assignment.
5. Return to Operations, download the signed PDF, and run the recovery drill to see retries, a failed state, and successful operator replay.
...
NestJS ──► MySQL / RDS
  │
  ├──────► SQS ──► Worker ──► private S3 packets
  ├──────► signed S3 downloads
  └──────► Open-Meteo forecast API
```

Compose runs the AWS SDK paths against LocalStack, so local verification uses the same S3 and SQS contracts intended for AWS. The API persists each job, sends its ID to SQS, and returns immediately. A separate Nest application-context worker updates attempt state, performs idempotent packet storage, and lets SQS redrive poison messages after three receives. Retrying a failed drill job reuses the durable job record and clears only the synthetic failure flag.
...
- Generated packets remain private in S3-compatible storage; downloads use five-minute signed URLs and stored objects include a SHA-256 checksum.
